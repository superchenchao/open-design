...
- `packages/launcher-proto`: TypeScript launcher activation protocol. Owns launcher-compatible `launcher.json`/`runtime.json` DTOs, app/mode/source constants, namespace validation, TCP loopback `endpoint` validation, and descriptor builders for tools-dev/tools-pack parity. It must not import `sidecar-proto` or expose old `ipc` semantics.
...
- For new shared types, choose the boundary first: web/daemon app DTOs go in `contracts`; legacy sidecar IPC protocol goes in `sidecar-proto`; launcher activation descriptors go in `launcher-proto`; generic runtime code goes in `sidecar`; generic OS/process code goes in `platform`.
